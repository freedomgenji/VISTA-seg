...
SAVEPATH="${SAVEPATH:-./runs/metaviewer_v1_crop085_et4_tc3_wt3_bs2_eval100_ep700}"
...
if [[ -n "${RESUME_TRAIN:-}" ]]; then
  if [[ ! -f "${RESUME_TRAIN}" ]]; then
    ...
    echo "Set RESUME_TRAIN to the real checkpoint path, for example /root/2023/metaviewer_full_meta/runs/xxx/model_last.pth" >&2
    exit 1
  fi
