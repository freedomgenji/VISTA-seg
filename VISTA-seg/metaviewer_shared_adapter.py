...
class RWConv1d(nn.Conv1d):
    """View-aware Conv1d adapted from xxLifeLover/MetaViewer."""
...
class ConvBlock(nn.Module):
    """Conv1d + BN + ReLU block from the GitHub MetaViewer MetaNet."""
...
class SpatialMetaNet(nn.Module):
    """Original MetaViewer MetaNet applied token-wise to 3D feature maps.

    Original MetaViewer receives normalized view embeddings shaped [B, V, D].
...
class MetaViewerSharedAdapter(nn.Module):
    def __init__(self, channels=128, num_modalities=4, meta_channels=None, meta_kernels=3):
        super(MetaViewerSharedAdapter, self).__init__()
    ...
    def initial_metaviewer_params(self):
    ...
    def adapt_metaviewer(self, support_features, support_mask,
                         inner_steps=1, inner_lr=1.0e-3, first_order=True):
        """Inner loop that really adapts the MetaViewer adapter on support samples.
        ...
        """
        params = self.initial_metaviewer_params()
    ...
    def meta_forward(self, features, mask, support_ratio=0.5, inner_steps=1,
        ...
        """Full support-query MetaViewer path.

        Support samples first adapt the MetaViewer adapter. The adapted adapter
        ...
        fast_params, support_inner_loss = self.adapt_metaviewer(
            support_features,
            support_mask,
