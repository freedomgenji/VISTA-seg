...
class region_aware_modal_fusion_wmh(nn.Module):
    def __init__(self, in_channel=64, norm='in', num_cls=4, num_modal=4):
        super(region_aware_modal_fusion_wmh, self).__init__()
        self.num_cls = num_cls

        self.modal_fusion = nn.ModuleList([modal_fusion(in_channel=in_channel, num_modal=num_modal) for i in range(num_cls)])
        self.region_fusion = region_fusion(in_channel=in_channel, num_cls=num_cls)
        self.short_cut = nn.Sequential(
    ...
    def forward(self, x, prm, mask):
        ...
        modal_feat = torch.stack((flair, t1), dim=1)
        region_feat = [modal_feat[:, :, i, :, :] for i in range(self.num_cls)]
