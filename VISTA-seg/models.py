...
from layers import *
from vista_seg_shared_adapter import VISTASegSharedAdapter
...
class Decoder_fuse(nn.Module):
    ...
    def forward(self, x1, x2, x3, x4, mask, bottleneck_override=None, bottleneck_mask=None):
        ...
        # x4 is [B, M, 128, H/8, W/8, D/8]; VISTA-Seg mode swaps only this fused shared tensor.
...
class VISTASeg(nn.Module):
    """Full-meta VISTA-Seg BraTS segmentation model.

    During training, support samples adapt the VISTA-Seg adapter and the decoder
    receives the support-adapted bottleneck representation.
    """

    def __init__(self, num_cls=4, fusion_type='RFM', shared_rep_method='vista_seg',
                 meta_support_ratio=1.0, meta_inner_steps=1, meta_inner_lr=1.0e-3,
                 meta_first_order=True):
        super().__init__()
        if shared_rep_method != 'vista_seg':
            raise ValueError('Only shared_rep_method="vista_seg" is supported in this cleaned model.')
        if fusion_type != 'RFM':
            raise ValueError('Only fusion_type="RFM" is supported by VISTASeg.')

        self.shared_rep_method = 'vista_seg'
        ...
        self.vista_seg_adapter = VISTASegSharedAdapter(channels=basic_dims*8, num_modalities=4)
    ...
    def _run_vista_seg_adapter(self, features, mask):
        if self.is_training:
            return self.vista_seg_adapter.meta_forward(
                features,
                mask.bool(),
        ...
        z_support, weights, common_target = self.vista_seg_adapter.aggregate(features, mask.bool())
        return {
            "z_shared": z_support,
    ...
    def forward(self, x, mask):
        ...
        meta_outputs = self._run_vista_seg_adapter(x4, mask)
...
class VISTASegWMH(nn.Module):
    def __init__(self, num_cls=1, fusion_type='RFM', activation='sigmoid', shared_rep_method='vista_seg',
                 meta_support_ratio=1.0, meta_inner_steps=1, meta_inner_lr=1.0e-3,
                 meta_first_order=True):
        super().__init__()
        if shared_rep_method != 'vista_seg':
            raise ValueError('Only shared_rep_method="vista_seg" is supported in this cleaned model.')
        if fusion_type != 'RFM':
            raise ValueError('Only fusion_type="RFM" is supported by VISTASegWMH.')
        ...
        self.flair_encoder = Encoder()
        self.t1_encoder = Encoder()
        self.vista_seg_adapter = VISTASegSharedAdapter(channels=basic_dims*8, num_modalities=2)
